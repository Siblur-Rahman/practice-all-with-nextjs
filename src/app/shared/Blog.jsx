...
const Blog = ({id, docs, topic,mark, fileName, npmCode, heading, define, defineWithIndex, url, video, time, code, link}) => {
    return (
        <>
        {mark && <h2 className='bg-amber-300 text-2xl'>{mark}</h2>}
        {id && <div><h2 className='id to_jump' id={id}>{id} {docs && <a href={docs} target='_blank' className='text-red-500 btn-success btn'>Docs</a>}</h2></div>}   
